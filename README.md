...
```
git clone git@github.com:LeonardM01/graphql-example.git
```
...
Run the following command to create your database. This also creates the `User` and `Post` tables that are defined in [`prisma/schema.prisma`](./prisma/schema.prisma), additionally it seeds the database with some predefined data:
